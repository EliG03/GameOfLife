...
public class LandscapeDisplay {
    ...
    private class LandscapePanel extends JPanel {
        ...
        public void paintComponent(Graphics g) {
            ...
            drawLandscapeWithColors(g, gridScale);
    ...
    public void drawLandscapeWithColors(Graphics g, int scale){
        //iterates through the entire ladnscape
        for (int i = 0; i<scape.getRows(); i++){
            for (int j = 0; j<scape.getCols(); j++){
                if (scape.getCell(i, j).getAlive()){
                    g.setColor(Color.RED); //sets the alive cells to red
                }
                else{
                    g.setColor(Color.BLUE); //sets dead cells to blue
                }

                //setting the fill oval parameters
                int ovalWidth = scale;
                int ovalHeight = scale;
                int ovalX = i*scale;
                int ovalY = j*scale;
